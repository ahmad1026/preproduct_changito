...
import { TextField, Button, ListItem, ListItemButton, ListItemIcon, List } from '@mui/material';
            ...
            {['خانه','مستندات', 'درباره ما', 'قوانین استفاده'].map((text, index) => (
                <ListItem key={text} disablePadding className='text-white'>
                    <Link className='w-full' href={
                        index === 0 ? '/' : index == 1 ? '/api-doc' : index == 2 ? '/' : index == 3 ? '/terms' : ''
                    }>
        ...
        <>
                ...
                <div className='max-w-7xl mx-auto py-2 flex items-center justify-between'>
                    <div className='flex items-center gap-5'>
                        ...
                        <ul className='flex items-center gap-2 text-white'>
                            <Link className={`${pathName==='/' ? 'border': '' } gap-2 rounded-md px-4 py-1 flex items-center`} href={'/'} >
                                <Home size={18} />
                                <span className='text-sm'>خانه</span>

                            </Link>
                            <Link className={`${pathName==='/api-doc' ? 'border': '' } gap-2 rounded-md px-4 py-1 flex items-center`} href={'/api-doc'} >
                                <Terminal size={18} />
                                <span className='text-sm'>مستندات</span>

                            </Link>
                            <Link className={`${pathName==='/about-us' ? 'border': '' } gap-2 rounded-md px-4 py-1 flex items-center`} href={'/about-us'} >
                                <Info size={18} />
                                <span className='text-sm'>درباره ما</span>

                            </Link>
                            <Link className={`${pathName==='/terms' ? 'border': '' } gap-2 rounded-md px-4 py-1 flex items-center`} href={'/terms'} >
                                <Settings size={18} />
                    ...
                    <Button onClick={() => setAuthModal(true)} className='flex items-center gap-3' variant="outlined" color={'primary'}>
                        <span>ورود</span>
                        <span>|</span>
            ...
            <div className='w-full fixed top-0 left-0 z-50 bg-[#1c1c1c] flex md:hidden items-center justify-between p-4'>
                <div className='flex gap-2 items-center'>
                    <Hamburger size={22} color='#BB86FC' toggled={isOpen} toggle={setOpen} />
                    <h1 className='text-white'>اوکی<span>&#8202;</span>چنج</h1>
                </div>
                <Button onClick={() => setAuthModal(true)} className='flex items-center gap-3' variant="outlined" color={'primary'}>
                    <span>ورود</span>
                    <span>|</span>
